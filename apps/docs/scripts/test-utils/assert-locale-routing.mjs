...
export async function assertLocaleRouting(request) {
    const legacy = await request('/docs?q=react&page=2', { redirect: 'manual', headers: { 'Accept-Language': 'ko-KR' } })
    ...
    for (const locale of ['ko', 'en', 'ko']) {
        const response = await request(`/${locale}/about`, { headers: { Cookie: `NEXT_LOCALE=${locale === 'ko' ? 'en' : 'ko'}` } })
        assert.equal(response.status, 200)
        const html = await response.text()
        assert.match(html, new RegExp(`<html[^>]*lang="${locale}"`))
        assert.ok(html.includes(`rel="canonical" href="https://heap-forge.app/${locale}/about"`))
    ...
    assert.ok(xml.includes('https://heap-forge.app/ko/docs'))
    assert.ok(xml.includes('https://heap-forge.app/en/docs'))
    const denied = await request('/api/revalidate/content', { method: 'POST', redirect: 'manual' })
    ...
    await unknown.text()
    console.log('[PASS] Locale URLs, URL-over-cookie priority, legacy query redirect, canonical/hreflang, sitemap, API and 404')
}
