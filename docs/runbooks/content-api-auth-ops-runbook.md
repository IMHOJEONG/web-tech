...
## Local / Remote Loading Boundary

`apps/docs`는 로컬 문서와 원격 문서의 실패 경계를 분리한다.

목록/검색 정책:

- 로컬 문서는 항상 독립적으로 로드한다.
- 원격 문서 목록 요청이 실패해도 `/feed`, `/docs`, 검색 화면 전체를 오류 페이지로 보내지 않는다.
- 원격 실패 시 해당 요청은 빈 원격 목록으로 낮추고, 로컬 문서만으로 화면을 렌더링한다.
- 원격과 로컬이 같은 공개 route를 만들면 원격 문서를 우선 노출하고 중복 카드는 제거한다.
