...
HeapForge docs 앱은 이러한 요청이 애플리케이션 라우팅이나 원격 콘텐츠 로딩으로 이어지지 않도록 앱 레벨 Proxy에서 빠르게 차단한다.
...
2. 저장소에 남겨야 하는 공통 정책은 Proxy blocklist에 추가한다.
