...
2026-09-21 적용: 첫 글은 큰 이미지 hero 대신 제목 중심의 compact 카드로 표시한다.

- `MainFeed`는 기존 필터와 첫 글 선택, 상세 URL 계산을 담당한다.
- `FeedLeadStory`는 피드 상단 배치와 번역 문구를 담당한다.
- `entities/document`의 `DocumentPreviewCard`는 제목·요약·주제·이미지와 단일 링크를 표시한다.
  제목 단계는 기본 `h2`이며 피드 상단에서만 `h1`을 지정한다. 데이터 조회나 피드 필터에는 의존하지 않는다.
- `DocumentCardMeta`는 작성자·읽기 시간을 표시하고, 이미지는 기존 `shared/ui/DocumentThumbnail`을 재사용한다.
- 카드 전체를 하나의 링크로 만들어 이미지·여백 클릭, Enter, 새 탭 열기가 같은 URL을 사용한다.
  링크 안에 별도 버튼이나 링크를 중첩하지 않는다. 제목을 링크 이름으로 사용하고 중복 썸네일은 장식 이미지로 처리한다.
- 이미지 높이는 144/160/192px로 제한하고 768px 이상부터 텍스트 옆에 배치한다.
  제목은 줄바꿈을 허용하며 요약만 세 줄로 제한한다. 기존 단독 CTA와 중복 배경 이미지는 제거한다.
- 최근 글 목록도 같은 `DocumentPreviewCard`를 `h3` 제목으로 사용한다.
  상단 글을 제외한 최대 4개 글을 기존 순서로 보여주고, 실재하지 않는 예시 카드는 만들지 않는다.
- `feed-document` model은 메타데이터 가공과 필터 선택을, `FeedFilters`는 query 링크를,
  `FeedRecentArticles`는 목록과 빈 상태를 담당한다. `MainFeed`는 이 구성요소를 조합한다.
- 원격 데이터 조회, 필터 query, 상세 라우팅 정책은 변경하지 않는다.

검사 범위는 [피드 상단 카드 작업 기록](../worklog/2026-09/2026-09-21-feed-lead-card.md)을 참고한다.
