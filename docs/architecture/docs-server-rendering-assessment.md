...
원격 응답 캐시나 렌더링 결과의 요청 간 캐시는 추가하지 않는다.

상세·검색·카테고리의 frontmatter 검증, 공개 상태 판단, 경로와 썸네일 정규화는
`local-document-parser.ts`의 순수 함수로 통합했다. 파일 탐색 범위와 동기/비동기
읽기는 각 로더에 남긴다. 따라서 파서 공통화가 로더 간 파일 읽기 캐시 통합을 뜻하지는 않는다.
검색 본문은 공통 파서 결과의 앞뒤 공백만 제거한다. frontmatter 제거 후 본문에
같은 정규식을 다시 적용하면 `---` 구분선 사이의 내용이 사라지므로 재적용하지 않는다.

`document.types.ts`의 문서 필드와 태그 배열은 readonly 계약을 사용한다.
이는 TypeScript 수준의 보호이며 객체의 런타임 deep freeze는 아니다.
Next.js 메타데이터처럼 변경 가능한 배열을 요구하는 외부 경계에서만 태그를 복사한다.
검색의 발췌 표시와 섹션 추론은 그대로 유지하고, 카테고리 결과에는 공통 파서가
`contentSource: local`, `contentFormat: mdx`도 제공한다.
